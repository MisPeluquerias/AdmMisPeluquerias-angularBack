...
router.get("/getAllReclamations", async (req, res) => {
  const {
    ...
    pageSize = "3",
    filterState,
  } = req.query as { page?: string; pageSize?: string; filterState?: string };
...
router.put("/updateStateReclamation", async (req, res) => {
  ...
  if (!id_salon_reclamacion || !state || !id_user || !salon_name) {
    return res
      ...
      .json({
        error:
          "Faltan datos requeridos: id_salon_reclamacion, state, id_user o salon_name",
      });
  }
  ...
  try {
    ...
    if (state === "Pendiente" || state === "En revision") {
      ...
      const updateSalonResults = await new Promise<OkPacket>(
        (resolve, reject) => {
          connection.query(
            updateSalonStateQuery,
            [id_salon],
            (error: unknown, results) => {
              if (error) return reject(error);
              resolve(results as OkPacket);
    ...
    if (state === "Reclamado") {
      try {
        ...
        if (salonResult.length === 0) {
          throw new Error(
            "No se encontró un salón con el nombre proporcionado."
          );
        }
        ...
        if (insertResults.affectedRows === 0) {
          throw new Error(
            "Error al insertar el registro en la tabla user_salon."
          );
        }
        ...
        if (email && email.trim() !== "") {
          ...
          console.error(
            "El correo electrónico del usuario no está definido o es inválido."
          );
        }
      } catch (error) {
        console.error(
          'Error durante la actualización de la reclamación a "Reclamado":',
          error
        );
        throw error;
      }
    ...
    if (error instanceof Error) {
      console.error(
        "Error al actualizar la reclamación o permisos:",
        error.message
      );
    } else {
      console.error(
        "Error desconocido al actualizar la reclamación o permisos:",
        error
      );
    }
    connection.rollback(() => {
      res
        ...
        .json({
          error:
            "Ocurrió un error al actualizar la reclamación o los permisos del usuario.",
        });
    });
  }
});
