...
router.get("/exportSalonsToExcel", async (req, res) => {
  const query = `
    ...
    s.*,
    GROUP_CONCAT(DISTINCT TRIM(REPLACE(c.categories, ' ;', '')) SEPARATOR '; ') AS categories,
    GROUP_CONCAT(DISTINCT sv.name SEPARATOR ', ') AS services,
    GROUP_CONCAT(DISTINCT st.name SEPARATOR ', ') AS subservices
          FROM 
    salon s
LEFT JOIN 
    categories c ON s.id_salon = c.id_salon
LEFT JOIN 
    salon_service_type ss ON s.id_salon = ss.id_salon
LEFT JOIN 
    service sv ON ss.id_service = sv.id_service
LEFT JOIN 
    service_type st ON ss.id_service_type = st.id_service_type
GROUP BY 
    s.id_salon`;
  ...
  connection.query(query, async (error, results) => {
    ...
    try {
      ...
      worksheet.columns = [
        ...
        { header: "Categorías", key: "categories", width: 50 },
      ...
      worksheet
        ...
        .eachCell(
          { includeEmpty: true },
          (cell: ExcelJS.Cell, rowNumber: number) => {
            if (rowNumber > 1) {
              // Omite la cabecera
              cell.dataValidation = {
                type: "list",
                ...
                error:
                  "El valor debe ser uno de: Validado, Reclamado, No reclamado",
              };
            }
          }
        );
          ...
          (cell: ExcelJS.Cell, rowNumber: number) => {
            if (rowNumber > 1) {
              ...
              cell.dataValidation = {
                ...
                errorTitle: "Entrada no válida",
                error:
                  "El valor debe ser uno de: Barbería,Peluquería,Estética,Academia,Manicura y Pedicura,Depilación Láser",
              };
            }
          }
        );
